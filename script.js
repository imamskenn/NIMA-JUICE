(function() {
    const WA_NUMBER = '6281234567890'; // ganti sesuai nomor WA

    // Best Seller cards: isi otomatis supaya loop
    ...
    // buat loop: duplikat cards untuk marquee seamless
    ...
    document.querySelectorAll('button').forEach(btn=>{
        ...
        if(icon && (icon.innerText === 'add' || icon.innerText === 'add_shopping_cart')){
            btn.style.cursor = 'pointer';
            btn.onclick = ()=> {
                const name = btn.closest('div')?.querySelector('p')?.innerText || 'Menu Juice';
                window.open(`https://wa.me/${WA_NUMBER}?text=Order%20${encodeURIComponent(name)}`, '_blank');
            };
        }
    });
